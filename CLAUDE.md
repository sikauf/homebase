...
- `AUTH_PASSWORD` — enables password auth on all `/api` routes (login cookie or `Authorization: Bearer`). Unset locally, so dev and tests run with auth off. Set on the VPS.
